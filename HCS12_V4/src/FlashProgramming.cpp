...
//         getData32Target(buffer)-loadAddress);
   ...
   flashProgramHeader.copctlAddress = targetToNative16(headerPtr->copctlAddress);
   flashProgramHeader.clockFactor   = targetToNative32(headerPtr->clockFactor);
   ...
   print("FlashProgrammer::loadTargetProgram()\n"
         ...
         flashProgramHeader.clockFactor, flashProgramHeader.clockFactor,
         flashProgramHeader.copctlAddress,
         flashProgramHeader.flashData,
         flashData.data,
         flashData.size
         );
...
USBDM_ErrorCode FlashProgrammer::executeTargetProgram(FlashData_t *flashHeader, uint32_t size) {
   ...
   print("FlashProgrammer::executeTargetProgram()\n"
         ...
         targetToNative16(flashHeader->controller),
         ...
         targetToNative16(flashHeader->data)
         );
...
//      USBDM_ReadMemory(1,sizeof(buffer), flashProgramHeader.loadAddress, buffer);
   ...
   ReadMemory(memorySpace, sizeof(stackBuffer), ????, stackBuffer);
#endif
   return rc;
}
...
USBDM_ErrorCode FlashProgrammer::determineTargetSpeed(void) {
   ...
   targetBusFrequency = 20*int(0.5+((200.0 * timingDataResult.timingCount)/flashProgramHeader.clockFactor));
...
USBDM_ErrorCode FlashProgrammer::eraseFlash(void) {
   ...
   for (int index=0; ; index++) {
      ...
      targetFlashData.controller = nativeToTarget16(flashData.controller);
...
USBDM_ErrorCode FlashProgrammer::doFlashBlock(FlashImage     *flashImage,
   ...
   while (blockSize>0) {
      ...
      buffer.targetFlashData.controller = nativeToTarget16(flashData.controller);
      ...
      buffer.targetFlashData.data       = nativeToTarget16(flashData.data);
#if (TARGET == MC56F80xx)
      if (memoryType == MemXROM) {
         // |0x80 => XROM, |0x03 => Bank1 (Data)
