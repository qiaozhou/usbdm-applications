 ...
 */
IMPLEMENT_DYNAMIC_CLASS( HCS12Unsecure, wxDialog )
...
private:
   static const U32 flashControl=0x0100;
   static const U32 eepromControl=0x0110;

public:
   static int getBDMtoBUSFactor()        { return 1; }
   static U32 getNVSECAddress()          { return 0xFF0F; }
   static U32 getCOPCTL()                { return 0x003C; }

   static U32 getFCLKDIVAddress()        { return flashControl+0x0; }
   static U32 getFSECAddress()           { return flashControl+0x1; }
   static U32 getFTSTMODAddress()        { return flashControl+0x2; }
   static U32 getFCNFGAddress()          { return flashControl+0x3; }
   static U32 getFPROTAddress()          { return flashControl+0x4; }
   static U32 getFSTATAddress()          { return flashControl+0x5; }
   static U32 getFCMDAddress()           { return flashControl+0x6; }
   static U32 getFADDRAddress()          { return flashControl+0x8; }
   static U32 getFDATAAddress()          { return flashControl+0xA; }
   static U32 getECLKDIVAddress()        { return eepromControl+0x0; }
   static U32 getEPROTAddress()          { return eepromControl+0x4; }
   static U32 getESTATAddress()          { return eepromControl+0x5; }
   static U32 getECMDAddress()           { return eepromControl+0x6; }
   static U32 getEADDRAddress()          { return eepromControl+0x8; }
   static U32 getEDATAAddress()          { return eepromControl+0xA; }
};

// Flash commands
//static const U8 mBlank      = 0x05; //!< Blank check
static const U8 mWordProg   = 0x20; //!< Byte program
//static const U8 mBurstProg  = 0x25; //!< Burst program
static const U8 mMassErase  = 0x41; //!< Mass erase
//static const U8 mPageErase  = 0x40; //!< Page erase

static const U8  HCS12_clearFlashErrors  = 0x32;    // PVIOL|ACCERR|FAIL
//static const U8  clearFlashErrors  = 0x30;    // FPVIOL|FACCERR
static const U8  startFlashCommand = 0x80;    // FCBEF
//static const U8  dummyFlashValue[] = {0xFF,0xFF,0xFF,0xFF};  // Dummy value for flash write
//static const U32 dummyFlashAddress = 0xFF80U; // Any address in flash

// EEPROM Control
//static const U16 HCS12_MEMSIZE_Address = 0x001C; //!< Address of Memory Size register
//static const U16 HCS12_PPAGE_Address   = 0x0030; //!< Program Page register
//static const U16 HCS12_EPAGE_Address   = 0x0017; //!< Program Page register
static const U8  HCS12_FTSTMOD_WRALL   = 0x10;   //!< FTSTMOD_WRALL bit mask

#define SOPT_BKGDPE_MASK         (1<<1)          // BKGD pin enable in SOPT(1)
#define FCLKDIV_FDIVLD           (1<<7)
#define ECLKDIV_EDIVLD           (1<<7)

static const U8  COPCTL_value   = 0x00;  // Value to initialise COPCTL - disable COP
...
//!
static U8 findFlashDividerValue(unsigned long fbus,
                                unsigned long fMin,
                                unsigned long fMax) {
...
USBDM_ErrorCode HCS12Unsecure::bulkEraseMemory() {
   const U8 allOnes        = 0xFF;
   const U8 allZeroes      = 0x00;
   U8 dummyFlashAddress[]  = {0xFF, 0xFE};
   U8 dummyEepromAddress[] = {0x0C, 0x00};
   U8 dummyFlashData[]     = {0xFF, 0xFF};
   int timeout;
   U8 statValue;
   USBDM_ErrorCode rc;
...
USBDM_ErrorCode HCS12Unsecure::programSecurityLocation() {
   ...
   U8 unsecureFlashValue[] = {0xFF, 0xFE};
   const U8 allOnes   = 0xFF;
   const U8 allZeroes = 0x00;
   int timeout;
   U8 statValue;
