...
// Dummy routines if logging is not required
   ...
   inline void openLogFile(const char *LogFileName, const char *description="USBDM Log File") { return; }
   inline void closeLogFile(void) { return; }
   inline void enableLogging(bool value) { return; }
   inline FILE *getFileHandle(void) {return NULL;}
   inline void setLogFileHandle(FILE *fp) {}
   inline FILE *getLogFileHandle(void) {return NULL;}
#else // LOG
...
void openLogFile(const char *LogFileName, const char *description="USBDM Log File");
