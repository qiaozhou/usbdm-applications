...
#endif
static const TargetType_t targetType = T_ARM_JTAG;
...
#if TARGET == ARM
         USBDM_ErrorCode rc = ARM_WriteMemory(memorySpace, sub, writeAddress, memoryReadWriteBuffer);
#elif TARGET == MC56F80xx
         USBDM_ErrorCode rc = DSC_WriteMemory(memorySpace, sub, writeAddress, memoryReadWriteBuffer);
#else
         USBDM_ErrorCode rc = USBDM_WriteMemory(memorySpace, sub, writeAddress, memoryReadWriteBuffer);
#endif
...
//   print("DiRegisterWrite(0x%X(%d) <= 0x%08X)\n", dnRegNumber, dnRegNumber, (uint32_t)value);
   ...
   switch (dnRegNumber) {
      ...
      case arm_RegCONTROL :
         ARM_ReadRegister(ARM_RegMISC, &dataValue);
         dataValue &= ~(0xFF<<24);
         dataValue |= (value&0xFF)<<24;
         rc = ARM_WriteRegister(ARM_RegMISC, dataValue);
         break;
      ...
      case arm_RegPRIMASK :
         ...
         break;
      default:
         rc = ARM_WriteRegister(regNum, value);
         break;
   }
...
ARM_Registers_t regNum = (ARM_Registers_t)dnRegNumber;
   ...
   switch (dnRegNumber) {
      // These regs are read together so need RMW sequence
      case arm_RegCONTROL :
         rc = ARM_ReadRegister(ARM_RegMISC, &dataValue);
      ...
      case arm_RegPRIMASK :
         ...
         break;
      default:
         rc = ARM_ReadRegister(regNum, &dataValue);
         break;
   }
   if (rc != BDM_RC_OK) {
...
//   }
   ...
   // Reset on OK status
   failureCount = 0;
   if ((status.dhcsr & DHCSR_S_SLEEP) != 0) {
   ...
   else if ((status.dhcsr & (DHCSR_S_HALT|DHCSR_S_LOCKUP)) != 0) {
